...
describe Manifestation, solr: true do
  ...
  it 'shoulld get search_error in openurl' do
    -> { Openurl.new(isbn: '12345678901234') }.should raise_error(OpenurlQuerySyntaxError)
    -> { Openurl.new(issn: '1234abcd') }.should raise_error(OpenurlQuerySyntaxError)
    -> { Openurl.new(aufirst: 'テスト 名称') }.should raise_error(OpenurlQuerySyntaxError)
  end

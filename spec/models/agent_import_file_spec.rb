...
describe AgentImportFile do
  ...
  describe "when it is written in shift_jis" do
    before(:each) do
      @file = AgentImportFile.create!(user: users(:admin))
      @file.attachment.attach(io: File.new("#{Rails.root}/../fixtures/files/agent_import_file_sample3.tsv"), filename: 'attachment.txt')
    end
  ...
  describe "when its mode is 'destroy'" do
    it "should remove users" do
      ...
      file = AgentImportFile.create!(user: users(:admin))
      file.attachment.attach(io: File.new("#{Rails.root}/../fixtures/files/agent_delete_file.tsv"), filename: 'attachment.txt')
      file.remove

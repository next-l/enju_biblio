...
resource_import_file_test1	007.6	note for the item.
        EOF
        file = ResourceImportFile.create(user: users(:admin))
        file.attachment.attach(io: StringIO.new(import_file), filename: 'attachment.txt')
        old_manifestations_count = Manifestation.count
        old_items_count = Item.count
        result = file.import_start
...
resource_import_file_test_description	test\\ntest	test\\ntest	test_description	test\\ntest
  ...
  describe "when its mode is 'update'" do
    ...
    #  file = ResourceImportFile.create resource_import: File.new("#{Rails.root.to_s}/../fixtures/files/update_series_statement.tsv"), edit_mode: 'update'
    #  file.modify
  ...
  it "should import in background", vcr: true do
    file = ResourceImportFile.create(user: users(:admin))
    file.attachment.attach(io: File.open("#{Rails.root}/../fixtures/files/resource_import_file_sample1.tsv"), filename: 'attachment.txt')
